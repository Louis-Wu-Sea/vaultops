---
name: vault:context
description: Show full project context — current stage, active tasks, recent journal, and scope verification. Use when the user says "context", "what's going on", "project status", "check context", or "/vault:context".
user-invocable: true
...
# /vault:context — Project Context Verifier
...
- If NO active tasks: suggest "Consider creating a task with /vault:task before starting work"
- If work doesn't match any task: warn "Current work doesn't match any tracked task. Create one with /vault:task?"
